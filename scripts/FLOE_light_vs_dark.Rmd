...
compare_lrt <- glmLRT(fit, coef = colnames(design[,-1:-4]))
```
...
write.csv(DEgene_LD_compare,"../output/DEgenes_LD_compare.csv")
```

```{r or if you want to keep all of them}
DEgene_LD_compare_all <- topTags(compare_lrt, n = Inf, p.value = 1)$table
write.csv(DEgene_LD_compare_all,"../output/DEgenes_LD_compare_all.csv")
```
