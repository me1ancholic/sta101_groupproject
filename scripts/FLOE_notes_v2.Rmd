...
dry_sdiv <- sdiv %>%
  select(contains("01") | contains("02") | contains("03") | contains("04") | contains("05"))
# would be curious to see how you would've done this part
```
...
- no ptg000013l since that was added after the blast
  - i gave it a name - use diff to compare lines or use coordinates: Sdiv_ptg000013l_FLOE3-R

# Week 4

### WEEK 4 FINISHED IN FLOE_notes_v3.Rmd

- are these floe ones sig or notsig?
- germination proportion - copy and paste what we already have to wet samples
- will be sent set/germination data
- column_to_rownames() add it in before you do things, compare to lab to see how they did it
      - do read.tsv instead maybe and no need for [-1] in code

## Fixing Row Names

```{r adding rownames attempt #1, eval=FALSE}
sdiv1 <- read.tsv("../input/Christina_HiFi_Sdiv_Gene_Counts.tsv")
# read.tsv is not a command
```

```{r adding rownames attempt #2, eval=FALSE}
sdiv_rownames <- sdiv[[1]]
sdiv2 <- sdiv[,-1]
rownames(sdiv2) <- sdiv_rownames
sdiv2
# Warning: Setting row names on a tibble is deprecated.
# this also does not work
```

```{r adding rownames attempt #3, eval=FALSE}
sdiv3 <- sdiv
rownames(sdiv3) <- sdiv3[[1]]
# Warning: Setting row names on a tibble is deprecated.
# this also does not work
```

```{r adding rownames attempt #4, eval=FALSE}
sdiv4 <- read_tsv("../input/Christina_HiFi_Sdiv_Gene_Counts.tsv", col_names = TRUE, row.names(1))
rownames(sdiv4)
# this also does not work
```

```{r adding rownames attempt #5, eval=FALSE}
sdiv5 <- sdiv %>%
  rename("rowname" = 1) %>%
  column_to_rownames()
# this did work
...
```{r rearranging dry_sdiv names again}
dry_sdiv <- dry_sdiv %>%
  select(sort(colnames(dry_sdiv))) %>%
  select(gene_id, everything())
names(dry_sdiv)
```

```{r checking names again again}
names(dry_sdiv[,-1]) == dry_sample$sample
```
## Normalizing and Plotting

```{r calculate normalization factors again again}
dge_data <- DGEList(counts = dry_sdiv[,-1],
        samples=dry_sample,
        group = dry_sample$group)
...
```{r plotting dge_data again again}
plotMDS(dge_data, method = "bcv")
# why did this change so much
```

```{r extracing normalized data again again}
normal_dry_sdiv <- cpm(dge_data)
normal_dry_sdiv_log <- cpm(dge_data, log = T)
```
...
```{r doing cor() again}
cor_table <- dry_sdiv %>% select(-gene_id) %>% cor()
cor_table %>% gplots::heatmap.2(Rowv=FALSE, Colv=FALSE, dendrogram="none", trace = "none", col=viridis::viridis(25, begin=.25), margins=c(7,8))
```

## Calculating Dispersion Factors (with trimming)
