...
```{r correlation between floe expression and germination attempt #1}
...
```{r correlation between floe expression and germination attempt #2}
...
cor_floe2 <- dry_sdiv_normal_log2 %>%
  ...
  mutate(CAAN1 = mean(c(a02, a03, a04, a05)),
         ...
         STTO_BH = mean(c(m01 ,m02, m03, m04, m05))) %>%
  select(rowname, CAAN1, CAAN2, CACO1, CAIN2, STBR3, STDI, STGL1, STGL2, STGL3, STIN, STPO1, STTO_BH) %>%
...
```{r correlation between floe expression and germination attempt #3}
...
# I'm not sure if this was what we're trying to look for, especially since the values vary so much for each gene
# You can kind of see that lower log2cpm has higher germination in graph 2, but I'm not sure if that is significant
# I think part of the confusion is what data i should be putting into the graph with regards to expression level
cor_floe2 %>%
...
lm(cor_floe2$log2cpm ~ cor_floe2$cumulative_prop_germ)
```
